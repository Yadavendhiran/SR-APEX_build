...
video=cv2.VideoCapture(r"A:\rec\gif\peace sign.mp4")
device=check_1080_480_srcnn.device
initial_total_time=time.perf_counter() # total time
model=SRCNN()
model.load_state_dict(torch.load("SRCNN_1.5X_v2.pth",map_location="cpu"))    
model.eval()
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
fps=video.get(cv2.CAP_PROP_FPS)
video_writer=cv2.VideoWriter("output_720_9.mp4",fps=fps,frameSize=(1280,720),fourcc=fourcc)
model_time=0
tranform_time=0
write_time=0
frame_count=0
